...
object ViewWiseBalancedEquality {
  ...
  sealed trait Equality[L, R] extends Any {
  ...
  object Equality extends LowPriorityEqualityImplicits {
     ...
     */
    implicit def rightToLeftEquality[L, R](implicit view: R => L): Equality[L, R] = new RightToLeftViewEquality(view)
  }
  ...
  trait LowPriorityEqualityImplicits {
     ...
     */
    implicit def leftToRightEquality[L, R](implicit view: L => R): Equality[L, R] = new LeftToRightViewEquality(view)
  }

  private class RightToLeftViewEquality[L, R](val view: R => L) extends Equality[L, R] {
    override def areEqual(left: L, right: R) = left == view(right)
  }

  private class LeftToRightViewEquality[L, R](val view: L => R) extends Equality[L, R] {
    override def areEqual(left: L, right: R) = view(left) == right
  }
}
